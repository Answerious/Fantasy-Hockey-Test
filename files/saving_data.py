...
class Create_Saved_Data:
    @classmethod
    def __init__(self):
        self.PLAYERDATA = open("saved_data/Player_Data", "w+")
        self.SCHEDULEDATA = open("saved_data/Schedule_Data", "w+")
        self.DRAFTORDERDATA = open("saved_data/Draft_Data", "w+")
        self.TEAMSELECTEDDATA = open("saved_data/Save#1_Data","w+")
        self.SELECTED_TEAM = ""
    ...
    def Draft_Order_Data(self):
        for n in self.DRAFT: self.DRAFTORDERDATA.write("{0}\n".format(n))
