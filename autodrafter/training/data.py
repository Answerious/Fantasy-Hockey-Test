...
for x in raws[0]:
    start += 1
    starts = root.createElement('value'+str(start))
    dov.appendChild(starts)
    starts.setAttribute('value','{0}'.format(x))
...
for x in raws[1]:
    gone += 1
    gones = root.createElement('value'+str(gone))
    gov.appendChild(gones)
    gones.setAttribute('value','{0}'.format(x))
